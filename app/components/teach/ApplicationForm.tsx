...
import { TEACHING_CATEGORY_OPTIONS } from '@/lib/constants';
...
import { UAE_CITIES } from '@/lib/utils';
...
const ApplicationForm = ({
  ...
  const onSubmit = async (data: z.infer<typeof createApplicationSchema>) => {
    ...
    setTimeout(() => {
      router.push('/teach/apply/payments/setup');
    }, 200);
  };
            ...
            render={({ field, fieldState }) => (
              <Field data-invalid={fieldState.invalid}>
                ...
                <MultipleSelector
                  ...
                  value={TEACHING_CATEGORY_OPTIONS.filter((option) =>
                    field.value.includes(option.value),
                  )}
                  onChange={(options) =>
                    field.onChange(options.map((opt) => opt.value))
                  }
                  defaultOptions={TEACHING_CATEGORY_OPTIONS}
